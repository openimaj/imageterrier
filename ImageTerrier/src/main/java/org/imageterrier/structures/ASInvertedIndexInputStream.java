 ...
 * under the License.
 *
 * The Original Code is ASInvertedIndexInputStream.java
 *
...
public class ASInvertedIndexInputStream extends InvertedIndexInputStream implements IndexConfigurable {
	protected int DocumentBlockCountDelta = 1;

	public ASInvertedIndexInputStream(Index index, String structureName, Class<? extends IterablePosting> postingIteratorClass) throws IOException {
		super(index, structureName, postingIteratorClass);
	}

	public ASInvertedIndexInputStream(Index index, String structureName, Iterator<? extends Pointer> lexInputStream, Class<? extends IterablePosting> postingIteratorClass) throws IOException {
		super(index, structureName, lexInputStream, postingIteratorClass);
	}

	public ASInvertedIndexInputStream(Index index, String structureName, Iterator<? extends Pointer> lexInputStream) throws IOException {
		super(index, structureName, lexInputStream);
	}

	public ASInvertedIndexInputStream(Index index, String structureName) throws IOException {
...
//		int previousTermId = -1;
		for (int i=0; i<numOfTerms; i++) {
			//tmpBlocks[i] = Utils.fromIndex(previousTermId = file.readGamma() + previousTermId);
			simulations[i] = file.readBinary(32);
		}
		...
		for (int i = 1; i < df; i++) {					
			...
			numOfTerms = documentTerms[3][i] = file.readUnary() - DocumentBlockCountDelta;
			simulations = new int[numOfTerms];
			
//			previousTermId = -1;
			for (int k=0; k<numOfTerms; k++) {
				//tmpBlocks[k] = Utils.fromIndex(previousTermId = file.readGamma() + previousTermId);
